...
def approve_users(modeladmin, request, queryset):
    ...
    for user in queryset.exclude(status="blocked"):
        ...
        if user.email:
            try:
                ...
                # Avoid breaking admin approval flow because of email issues
                pass

    message = f"{updated} user(s) approved successfully."
    if emailed:
        message += f" Approval email sent to {emailed} user(s)."
...
def approve_kyc(modeladmin, request, queryset):
    ...
    message = f"{updated} KYC profile(s) approved successfully."
    if emailed:
        message += f" Approval email sent to {emailed} user(s)."
...
class UserAdmin(BaseUserAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.status == "blocked":
            obj.is_active = False
        elif obj.status in ["pending", "approved"]:
            obj.is_active = True
