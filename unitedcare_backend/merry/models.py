...
class MerryGoRound(models.Model):
    ...
    def get_slot_due_date(self, period_key: str, slot_no: int) -> Optional[date]:
        ...
        if slot_no == 1:
            ...
            if seat and seat.payout_day is not None:
...
class MerryMember(models.Model):
    ...
    def __str__(self):
        return f"user-{self.user_id} - merry-{self.merry_id}"
# ----------------------------
# Seats/Shares
# ----------------------------
...
class MerryPayment(models.Model):
    ...
    # Net merry contribution amount only.
    # This is the amount used for due allocation and merry collection totals.
    amount = models.DecimalField(max_digits=12, decimal_places=2)

    # Total amount received from the payment channel before separating charges.
    # For old rows this defaults safely to amount when saved through this model.
    gross_amount = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal("0.00"))

    # Payment channel or transaction fee.
    # This must never be allocated to dues or credited as wallet excess.
    transaction_fee = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal("0.00"))
    ...
    def clean(self):
        ...
        if self.gross_amount is not None and self.gross_amount < 0:
            raise ValidationError("gross_amount cannot be negative")

        if self.transaction_fee is not None and self.transaction_fee < 0:
            raise ValidationError("transaction_fee cannot be negative")

        if self.amount is not None and self.transaction_fee is not None and self.gross_amount is not None:
            expected_minimum_gross = q2(self.amount) + q2(self.transaction_fee)
            if q2(self.gross_amount) not in (Decimal("0.00"), expected_minimum_gross):
                if q2(self.gross_amount) < q2(self.amount):
                    raise ValidationError("gross_amount cannot be less than amount")
    ...
    def save(self, *args, **kwargs):
        ...
        # old callers may only pass amount. In that case, gross_amount follows amount.
        if self.amount is not None:
            self.amount = q2(self.amount)

        self.transaction_fee = q2(self.transaction_fee or Decimal("0.00"))

        if not self.gross_amount or q2(self.gross_amount) == Decimal("0.00"):
            self.gross_amount = q2((self.amount or Decimal("0.00")) + self.transaction_fee)
        else:
            self.gross_amount = q2(self.gross_amount)

        super().save(*args, **kwargs)

    def net_contribution_amount(self) -> Decimal:
        return q2(self.amount or Decimal("0.00"))

    def fee_amount(self) -> Decimal:
        return q2(self.transaction_fee or Decimal("0.00"))

    def __str__(self):
        return (
            f"Payment#{self.id} merry={self.merry_id} "
            f"member={self.beneficiary_member_id} {self.status} "
            f"amount={self.amount} fee={self.transaction_fee} gross={self.gross_amount}"
        )
