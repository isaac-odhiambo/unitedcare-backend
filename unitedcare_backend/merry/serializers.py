...
class CreatePaymentIntentSerializer(serializers.Serializer):
    # Net merry contribution only. Do not include transaction fee here.
    amount = serializers.DecimalField(max_digits=12, decimal_places=2)

    # Optional fields for payment providers that report a charge separately.
    gross_amount = serializers.DecimalField(max_digits=12, decimal_places=2, required=False)
    transaction_fee = serializers.DecimalField(max_digits=12, decimal_places=2, required=False, default=Decimal("0.00"))
    ...
    def validate(self, attrs):
        ...
        fee = q2(attrs.get("transaction_fee") or Decimal("0.00"))
        if fee < 0:
            raise serializers.ValidationError({"transaction_fee": "Transaction fee cannot be negative."})
        attrs["transaction_fee"] = fee
        ...
        if gross is None:
            ...
            gross = q2(gross)
            if gross < amt:
                raise serializers.ValidationError({"gross_amount": "Gross amount cannot be less than amount."})
        attrs["gross_amount"] = gross
