...
@transaction.atomic
def _auto_apply_wallet_to_member_reached_dues(
    *,
    member: MerryMember,
    reference: str = "",
    narration: str = "",
    ...
    """
    Automatically use a member's Merry wallet only for dues that have reached today
    or are overdue. Future dues are not touched.

    This helper does not call ensure_member_dues_up_to_current_turn() to avoid
    recursive wallet application. It simply uses already-created dues for this
    member and merry.
    """
    if not member or not getattr(member, "user_id", None):
        return {
    ...
    dues = list(
        ...
        .select_related("seat", "merry", "payout", "seat__member", "seat__member__user")
        .order_by("due_date", "seat__seat_no", "id")
    )

    _refresh_penalties_for_queryset(dues)

    remaining = wallet_balance
    ...
    if used_total > 0:
        ...
        _create_wallet_tx(
            ...
            balance_before=before,
            balance_after=after,
            reference=reference or f"AUTO-MERRY-{member.merry_id}",
            narration=narration or "Wallet automatically used for reached Merry due.",
        )
...
def ensure_member_dues_up_to_current_turn(*, member: MerryMember) -> int:
    ...
    created = _ensure_historical_member_dues_until_turn(member, getattr(payout, "turn_no", 1))

    # Automatically use the Merry wallet only when the due has reached today
    # or is overdue. Future dues are protected by due_date__lte in the helper.
    _auto_apply_wallet_to_member_reached_dues(
        member=member,
        reference=f"AUTO-MERRY-{member.merry_id}",
        narration="Wallet automatically used for current Merry due.",
    )
...
def _merry_payment_has_field(field_name: str) -> bool:
    try:
        MerryPayment._meta.get_field(field_name)
        return True
    except Exception:
        return False


def _get_mpesa_transaction_fee(mpesa_tx=None) -> Decimal:
    if mpesa_tx is None:
        return Decimal("0.00")
    ...
    for name in candidate_names:
        value = getattr(mpesa_tx, name, None)
        if value in (None, ""):
            continue
        try:
            fee = q2(value)
...
def _create_confirmed_payment_shell(
    *,
    ...
    net_amount = q2(total_amount)
    fee_amount = q2(transaction_fee or Decimal("0.00"))
    gross_paid_amount = q2(gross_amount if gross_amount is not None else net_amount + fee_amount)
    ...
    if _merry_payment_has_field("gross_amount"):
        create_kwargs["gross_amount"] = gross_paid_amount
    if _merry_payment_has_field("transaction_fee"):
        create_kwargs["transaction_fee"] = fee_amount

    return MerryPayment.objects.create(**create_kwargs)
...
def apply_mpesa_contribution_by_user_reference(
    *,
    ...
    transaction_fee = _get_mpesa_transaction_fee(mpesa_tx)
    if transaction_fee >= total_amount:
        raise BadState("transaction_fee cannot be greater than or equal to amount.")

    allocatable_amount = q2(total_amount - transaction_fee)
    remaining = q2(allocatable_amount)
    if remaining <= 0:
        raise BadState("Net merry contribution amount must be > 0.")

    fee_remaining = q2(transaction_fee)
    ...
    for member in members:
        ...
        fee_for_payment = fee_remaining
        fee_remaining = Decimal("0.00")

        payment = _create_confirmed_payment_shell(
            member=member,
            total_amount=pay_amount,
            gross_amount=q2(pay_amount + fee_for_payment),
            transaction_fee=fee_for_payment,
    ...
    return {
        ...
        "gross_amount": q2(total_amount),
        "transaction_fee": q2(transaction_fee),
        "allocated_amount": q2(allocatable_amount - remaining),
...
def create_payment_intent(*, user, merry_id: int, amount: Decimal, payer_phone: str) -> MerryPayment:
    ...
    if _merry_payment_has_field("gross_amount"):
        create_kwargs["gross_amount"] = amt
    if _merry_payment_has_field("transaction_fee"):
        create_kwargs["transaction_fee"] = Decimal("0.00")
...
# -----------------------------
# Mobile optimized merry detail
# -----------------------------
def _mobile_user_name(user) -> str:
    if not user:
        return ""
    return (
        getattr(user, "username", None)
...
def _mobile_seat_user_block(seat: MerrySeat) -> Dict[str, Any]:
    member = seat.member
    user = member.user
    ...
    return {
        ...
        "member_id": member.id,
        "user_id": user.id,
        "username": _mobile_user_name(user),
        "phone": getattr(user, "phone", None),
    }


def _mobile_merry_can_view(*, user, merry: MerryGoRound) -> bool:
    if not user or not getattr(user, "id", None):
        return False
    ...
    if getattr(merry, "is_open", False):
        return True

    if MerryJoinRequest.objects.filter(
        merry=merry,
        user=user,
    ).exists():
        return True
...
def _mobile_due_outstanding(due: MerryContributionDue) -> Decimal:
    return _outstanding_amount(due)


def _mobile_sum_due_rows(dues: List[MerryContributionDue]) -> Dict[str, Decimal]:
    due_total = Decimal("0.00")
    paid_total = Decimal("0.00")
    outstanding_total = Decimal("0.00")

    for due in dues:
        due_total = q2(due_total + q2(due.due_amount or Decimal("0.00")))
        paid_total = q2(paid_total + q2(due.paid_amount or Decimal("0.00")))
        outstanding_total = q2(outstanding_total + _mobile_due_outstanding(due))

    return {
        "due_total": q2(due_total),
        "paid_total": q2(paid_total),
        "outstanding_total": q2(outstanding_total),
    }


@transaction.atomic
def get_merry_mobile_detail(*, user, merry_id: int) -> Dict[str, Any]:
    """
    Lightweight mobile detail summary.

    This is intended for app/(tabs)/merry/[id].tsx first load.
    It returns:
    - merry summary
    - viewer/member summary
    - wallet/pay-now summary
    - current receiver
    - next receiver
    - admin readiness totals

    It does NOT return all paid/unpaid member rows.
    Rows should be loaded separately only when admin expands the list.
    """
    merry = get_merry(merry_id)

    if not _mobile_merry_can_view(user=user, merry=merry):
    ...
    is_member = active_member is not None

    # Current payout + next payout are the source of truth.
    payout = ensure_current_payout_exists(merry_id=merry.id)
    current_date = (
        getattr(payout, "scheduled_date", None)
        or _period_key_to_date(payout.period_key)
    ...
    # Keep current turn dues accurate without loading the full admin row list.
    ensure_dues_for_current_payout(merry_id=merry.id)

    # Existing service already returns current turn and next turn metadata.
    next_turn = get_next_payout_turn(merry_id=merry.id)

    current_seat = payout.seat
    current_receiver = _mobile_seat_user_block(current_seat)

    next_receiver = {
        "turn_no": next_turn.get("next_turn_no") or next_turn.get("next_turn"),
        "seat_id": next_turn.get("next_seat_id"),
        "seat_no": next_turn.get("next_seat_no"),
        "member_id": next_turn.get("next_member_id"),
        "user_id": next_turn.get("next_user_id"),
        "username": next_turn.get("next_username"),
        "phone": next_turn.get("next_phone"),
        "scheduled_date": next_turn.get("next_scheduled_date"),
        "slot_no": next_turn.get("next_slot_no"),
    }
    ...
    _refresh_penalties_for_queryset(current_due_rows)

    readiness_totals = _mobile_sum_due_rows(current_due_rows)
    ready_for_payout = (
        readiness_totals["due_total"] > Decimal("0.00")
        and readiness_totals["paid_total"] >= readiness_totals["due_total"]
    )

    active_seats_count = MerrySeat.objects.filter(
        merry=merry,
        is_active=True,
    ).count()

    latest_join_request = None
    if user and getattr(user, "id", None):
        latest_join_request = (
    ...
    if active_member:
        ...
        # Keep member dues accurate, but only for this viewer's own seats.
        ensure_member_dues_up_to_current_turn(member=active_member)
        ...
        _refresh_penalties_for_queryset(viewer_due_rows)

        for due in viewer_due_rows:
            viewer_due_now = q2(viewer_due_now + _mobile_due_outstanding(due))
    ...
    try:
        available_seats = merry.available_seats()
    except Exception:
        available_seats = None
    ...
    try:
        if available_seats is not None and available_seats > 0:
            available_seat_numbers = get_next_available_seat_numbers(
                merry,
                min(int(available_seats), 50),
            )
    except Exception:
        available_seat_numbers = []
    ...
    return {
        "merry": {
            ...
            "active_seats": active_seats_count,
            "available_seats": available_seats,
            "available_seat_numbers": available_seat_numbers,
            "penalty_mode": getattr(merry, "penalty_mode", "NONE"),
            "flat_penalty_amount": q2(
                getattr(merry, "flat_penalty_amount", Decimal("0.00"))
                or Decimal("0.00")
            ),
            "daily_penalty_amount": q2(
                getattr(merry, "daily_penalty_amount", Decimal("0.00"))
                or Decimal("0.00")
            ),
        ...
        "viewer": {
            ...
            "wallet_balance": q2(wallet_balance),
            "due_now": q2(viewer_due_now),
            "pay_now": q2(viewer_pay_now),
            "my_join_request": (
                {
                    "id": latest_join_request.id,
                    "status": latest_join_request.status,
                    "requested_seats": latest_join_request.requested_seats,
                    "created_at": latest_join_request.created_at,
                }
            ...
            "can_request_join": (
                not is_member
                and getattr(merry, "is_open", False)
                and (
        ...
        "current_turn": {
            ...
            "period_key": payout.period_key,
            "scheduled_date": current_date,
            "slot_no": getattr(payout, "slot_no", 1),
            "expected_amount": q2(payout.amount or Decimal("0.00")),
            "receiver": current_receiver,
        },
        "next_turn": next_receiver,
        "admin_readiness": {
            "payout_id": payout.id,
            "turn_no": getattr(payout, "turn_no", 1),
            "cycle_no": getattr(payout, "cycle_no", _current_cycle_number(merry)),
            ...
            "scheduled_date": current_date,
            "slot_no": getattr(payout, "slot_no", 1),
            "expected": readiness_totals["due_total"],
            "paid": readiness_totals["paid_total"],
            "outstanding": readiness_totals["outstanding_total"],
            "ready_for_payout": ready_for_payout,
            "payout_already_exists": True,
            "can_admin_create_payout": False,
...
@transaction.atomic
def get_merry_mobile_readiness_rows(*, user, merry_id: int) -> Dict[str, Any]:
    """
    Load full paid/partial/unpaid rows only when admin expands the list.
    This keeps the first Merry detail screen fast.
    """
    if not is_admin(user):
        raise NotAllowed("Admin only.")

    readiness = get_payout_readiness_status(merry_id=merry_id)
    ...
    return {
        "merry_id": readiness.get("merry_id"),
        "payout_id": readiness.get("payout_id"),
        "turn_no": readiness.get("turn_no"),
        "cycle_no": readiness.get("cycle_no"),
        "period_key": readiness.get("period_key"),
        "scheduled_date": readiness.get("scheduled_date"),
        "due_total": readiness.get("due_total"),
        "paid_total": readiness.get("paid_total"),
        "outstanding_total": readiness.get("outstanding_total"),
        "ready_for_payout": readiness.get("ready_for_payout"),
        "payout_already_exists": readiness.get("payout_already_exists"),
        "can_admin_create_payout": readiness.get("can_admin_create_payout"),
        "paid_rows": paid_rows,
