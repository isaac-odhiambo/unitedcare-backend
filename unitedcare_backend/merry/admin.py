...
class MerryJoinRequestAdmin(admin.ModelAdmin):
    ...
    def assigned_seats_display(self, obj):
        if not obj or not obj.pk:
            return "Save first to view assigned seats."

        if obj.status != "APPROVED":
            return "Seats will appear after approval."
        ...
        if not member:
            return "No active member record found."
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            ...
            if old_obj.status == "PENDING" and obj.status == "APPROVED":
                ...
                self.message_user(
                    request,
                    f"Join request approved. Seats: {', '.join(str(s.seat_no) for s in seats)}",
                    level=messages.SUCCESS,
                )
                return
